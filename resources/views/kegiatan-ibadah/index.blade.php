    ...
    <section class="panel">
        <div class="desktop-only table-wrap"><table class="employee-table"><thead><tr><th>Kegiatan</th><th>Kode</th><th>Jadwal</th><th>Status</th><th class="text-right">Aksi</th></tr></thead><tbody>
            @forelse ($kegiatanIbadah as $item)
                <tr><td><p class="person-name">{{ $item->nama }}</p><p class="person-meta">{{ $item->keterangan ?: 'Kegiatan ibadah siswa' }}{{ $item->khususLakiLaki() ? ' · Khusus siswa laki-laki' : '' }}</p></td><td><span class="badge badge-muted">{{ $item->kode }}</span></td><td>{{ $item->jumlah_jadwal_aktif }} aktif</td><td><span class="badge {{ $item->aktif ? 'badge-active' : 'badge-inactive' }}">{{ $item->aktif ? 'Aktif' : 'Nonaktif' }}</span></td><td><div class="actions" style="justify-content:flex-end;"><a href="{{ route('kegiatan-ibadah.show', $item) }}" class="button button-muted button-sm">Lihat</a><a href="{{ route('kegiatan-ibadah.edit', $item) }}" class="button button-dark button-sm">Edit</a></div></td></tr>
            @empty
        ...
        <div class="mobile-only mobile-list">
            @forelse ($kegiatanIbadah as $item)
                <article class="mobile-card"><div class="mobile-card-head"><div><p class="person-name">{{ $item->nama }}</p><p class="person-meta">{{ $item->kode }} &middot; {{ $item->jumlah_jadwal_aktif }} jadwal aktif</p></div><span class="badge {{ $item->aktif ? 'badge-active' : 'badge-inactive' }}">{{ $item->aktif ? 'Aktif' : 'Nonaktif' }}</span></div><p class="help-text" style="margin-top:10px;">{{ $item->keterangan ?: 'Kegiatan ibadah siswa' }}{{ $item->khususLakiLaki() ? ' · Khusus siswa laki-laki' : '' }}</p><div class="actions" style="margin-top:13px;"><a href="{{ route('kegiatan-ibadah.show', $item) }}" class="button button-muted">Lihat</a><a href="{{ route('kegiatan-ibadah.edit', $item) }}" class="button button-dark">Edit</a></div></article>
            @empty
