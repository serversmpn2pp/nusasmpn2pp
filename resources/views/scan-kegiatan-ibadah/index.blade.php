...
<head>
    ...
    <title>Scan Kegiatan Ibadah - NUSA</title>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .page { width:min(1180px,100%); margin:0 auto; padding:24px 20px calc(34px + env(safe-area-inset-bottom)); }
        .page-head { display:flex; align-items:flex-end; justify-content:space-between; gap:18px; margin-bottom:18px; }
        .eyebrow { margin:0 0 5px; color:var(--blue); font-size:.78rem; font-weight:800; }.page-head h1 { margin:0; font-size:clamp(1.55rem,3vw,2.1rem); line-height:1.12; }.page-head p:last-child { margin:8px 0 0; color:var(--muted); }
        .back-link { display:inline-flex; align-items:center; min-height:42px; padding:10px 15px; border:1px solid var(--line); border-radius:8px; background:#fff; color:var(--ink); font-weight:800; text-decoration:none; }
        ...
        .status-pill { flex:0 0 auto; padding:7px 10px; border-radius:999px; font-size:.75rem; font-weight:900; }.status-pill.aktif { background:var(--green-soft); color:var(--green); }.status-pill.belum,.status-pill.selesai,.status-pill.tidak_ada { background:#f3f4f6; color:#5d6672; }
        ...
        .camera-placeholder { position:absolute; inset:0; display:grid; place-content:center; gap:8px; padding:28px; color:#dbe8f4; text-align:center; }.camera-placeholder strong { color:#fff; font-size:1.15rem; }.camera-placeholder p { max-width:430px; margin:0; color:#a9bdd0; line-height:1.5; }
        .camera-wrap.camera-on .camera-placeholder { display:none; }
        .scan-frame { position:absolute; left:50%; top:50%; display:none; width:min(54%,260px); aspect-ratio:1; transform:translate(-50%,-50%); border:2px solid rgba(255,255,255,.9); border-radius:8px; box-shadow:0 0 0 999px rgba(2,12,24,.30); }
        .camera-wrap.camera-on .scan-frame { display:block; }.scan-frame::before,.scan-frame::after { position:absolute; content:""; width:38px; height:38px; border-color:var(--yellow); }.scan-frame::before { left:-3px; top:-3px; border-left:6px solid var(--yellow); border-top:6px solid var(--yellow); }.scan-frame::after { right:-3px; bottom:-3px; border-right:6px solid var(--yellow); border-bottom:6px solid var(--yellow); }
        ...
        .result { display:none; margin-top:18px; padding:16px; border:1px solid var(--line); border-left:5px solid var(--blue); border-radius:8px; background:#fff; box-shadow:var(--shadow); }.result.show { display:block; }.result.success { border-left-color:var(--green); background:var(--green-soft); }.result.known { border-left-color:var(--blue); background:var(--blue-soft); }.result.error { border-left-color:var(--red); background:var(--red-soft); }
        ...
        @media(max-width:820px) { .page { padding:18px 14px calc(30px + env(safe-area-inset-bottom)); }.page-head { align-items:flex-start; }.scan-layout { grid-template-columns:1fr; }.schedule-strip { grid-template-columns:1fr 1fr; }.schedule-block:first-child { grid-column:1/-1; }.camera-wrap { aspect-ratio:4/5; min-height:430px; }.side-column { display:flex; flex-direction:column; }.result { order:-1; margin:0 0 18px; } }
        @media(max-width:500px) { .app-header { min-height:64px; }.brand img { width:40px; height:40px; }.brand span { display:none; }.server-clock strong { font-size:1.08rem; }.page-head { display:block; }.back-link { width:100%; justify-content:center; margin-top:14px; }.schedule-strip { grid-template-columns:1fr 1fr; }.schedule-block { padding:13px; }.schedule-block:first-child { grid-column:1/-1; }.schedule-block.highlight { grid-column:1/-1; }.camera-wrap { min-height:0; aspect-ratio:3/4; }.camera-controls .button { flex:1 1 calc(50% - 9px); }.camera-controls .button-grow { flex-basis:100%; }.result-grid { grid-template-columns:70px minmax(0,1fr); }.result-photo { width:70px; height:86px; }.manual-form { flex-direction:column; }.manual-form .button { width:100%; } }
    </style>
</head>
<body>
    <header class="app-header">
        <a class="brand" href="{{ route('beranda') }}">
            ...
            <span><strong>NUSA Ibadah</strong><span>SMP Negeri 2 Padang Panjang</span></span>
        </a>
    ...
    <main
        id="scan-worship-app"
        class="page"
        data-endpoint="{{ route('scan-kegiatan-ibadah.store') }}"
    ...
    >
        <div class="page-head">
            <div><p class="eyebrow">Presensi Kegiatan Ibadah</p><h1>Scan QR Kartu Pelajar</h1><p>Arahkan kamera HP ke QR siswa setelah kegiatan selesai dilaksanakan.</p></div>
            <div style="display:flex;flex-wrap:wrap;gap:8px;"><a href="{{ route('rekap-kegiatan-ibadah.index') }}" class="back-link">Rekap hari ini</a><a href="{{ route('beranda') }}" class="back-link">Kembali ke NUSA</a></div>
        </div>
            ...
            <div>
                <section class="panel">
                    ...
                    <div id="camera-wrap" class="camera-wrap">
                        ...
                        <div class="camera-message">Posisikan QR di dalam kotak</div>
                    </div>
                    <div class="camera-controls">
                        <button id="start-camera" class="button button-primary button-grow" type="button" @disabled(!$scanDibuka)>Mulai kamera</button>
                        <button id="switch-camera" class="button button-secondary" type="button" disabled>Ganti kamera</button>
                        <button id="stop-camera" class="button button-danger" type="button" disabled>Hentikan</button>
                    </div>
                ...
                <details class="panel manual">
                    ...
                    <p class="manual-help">Pilihan cadangan untuk petugas. Masukkan NISN yang tercetak pada kartu pelajar.</p>
                    <form id="manual-form" class="manual-form">
                        <input id="manual-nisn" name="nisn" inputmode="numeric" autocomplete="off" placeholder="Masukkan NISN" @disabled(!$scanDibuka)>
                        <button class="button button-primary" type="submit" @disabled(!$scanDibuka)>Catat presensi</button>
                    </form>
                </details>
            </div>
            ...
            <div class="side-column">
                <section id="scan-result" class="result" aria-live="polite">
                    <div class="result-grid">
                        <img id="result-photo" class="result-photo" src="{{ asset('images/kartu-pelajar/default-user.png') }}" alt="Foto siswa">
                        <div><p id="result-kicker" class="result-kicker">Hasil scan</p><h3 id="result-name">Menunggu QR</h3><div id="result-meta" class="result-meta"></div><p id="result-text" class="result-text"></p></div>
                    </div>
                </section>

                <section class="panel">
                    <header class="panel-head"><div><h2>Presensi Terbaru</h2><p>Siswa yang sudah tercatat hari ini.</p></div><strong id="total-today" class="summary-number">{{ $jumlahHariIni }}</strong></header>
                    <div id="recent-list" class="recent-list">
                        @forelse($presensiTerbaru as $presensi)
                            <article class="recent-item" data-presence-id="{{ $presensi['id'] }}"><img class="recent-photo" src="{{ $presensi['foto_url'] ?: asset('images/kartu-pelajar/default-user.png') }}" alt=""><div><p class="recent-name">{{ $presensi['nama_lengkap'] }}</p><p class="recent-meta">{{ $presensi['kelas'] ?: '-' }} &middot; NISN {{ $presensi['nisn'] }}</p></div><time class="recent-time">{{ substr($presensi['waktu_scan'],0,5) }}</time></article>
                        @empty
                            <p class="recent-empty">Belum ada siswa yang tercatat hari ini.</p>
                        @endforelse
                    </div>
                </section>
            </div>
