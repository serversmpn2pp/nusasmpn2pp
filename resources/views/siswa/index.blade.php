...
@section('content')
    ...
    <form action="{{ route('siswa.index') }}" method="GET" class="panel panel-pad" style="margin-bottom: 24px;">
        <div class="filter-grid">
            <div class="field">
                <label for="kata_kunci">Cari siswa</label>
                <input id="kata_kunci" name="kata_kunci" type="search" value="{{ $kata_kunci }}" placeholder="Nama, NIS, NISN, atau NIK" class="input">
            </div>
            ...
            <div class="actions">
                ...
                <a href="{{ route('siswa.index') }}" class="button button-muted">Reset</a>
            </div>
        </div>
    </form>
        ...
        <div class="desktop-only table-wrap">
            <table class="employee-table">
                ...
                <tbody>
                    @forelse ($siswa as $item)
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="muted">NISN: {{ $item->nisn ?: '-' }}</div>
                            </td>
                            <td>
                                <div>{{ $item->tempat_lahir ?: '-' }}</div>
                                <div class="muted">{{ $item->tanggal_lahir ? $item->tanggal_lahir->format('d-m-Y') : '-' }}</div>
                            </td>
                            <td>
                                <div>Ayah: {{ $item->nama_ayah ?: '-' }}</div>
                                <div class="muted">Ibu: {{ $item->nama_ibu ?: '-' }}</div>
                            </td>
                            <td>
                                @if ($item->aktif)
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="empty-state">Belum ada data siswa.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
                    ...
                    <div class="mobile-card-main">
                        ...
                        <div style="min-width:0; flex:1;">
                            <div class="mobile-card-head">
                                <div>
                                    <p class="person-name">{{ $item->nama_lengkap }}</p>
                                    <p class="person-meta">{{ $item->tempat_lahir ?: '-' }}{{ $item->tanggal_lahir ? ', ' . $item->tanggal_lahir->format('d-m-Y') : '' }}</p>
                                </div>
    ...
    @if ($siswa->hasPages())
        <nav class="pagination-simple">
            ...
            <div class="actions">
                ...
                @else
                    <a href="{{ $siswa->previousPageUrl() }}" class="button button-muted">Sebelumnya</a>
                @endif

                @if ($siswa->hasMorePages())
                    <a href="{{ $siswa->nextPageUrl() }}" class="button button-muted">Berikutnya</a>
                @else
