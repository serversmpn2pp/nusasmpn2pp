...
<body>
    @php
        $jadwal = $ruangUjianCbt->jadwalUjianCbt;
        $mapel = $jadwal?->mataPelajaran?->nama ?: $ujianCbt->mataPelajaran?->nama;
        $fotoDefault = asset('images/kartu-pelajar/default-user.png');
    @endphp
    <header class="app-header">
        <a class="brand" href="{{ route('presensi-ujian-cbt.index') }}"><img src="{{ asset('images/logo-nusa.png') }}" alt="Logo NUSA"><span><strong>NUSA Ujian</strong><span>SMP Negeri 2 Padang Panjang</span></span></a>
    ...
    <main id="exam-attendance-app" class="page" data-endpoint="{{ route('presensi-ujian-cbt.scan', [$ujianCbt, $ruangUjianCbt]) }}" data-server-time="{{ $waktuServerIso }}" data-fallback-photo="{{ $fotoDefault }}" data-attendance-open="{{ $statusJendelaPresensi['dibuka'] ? '1' : '0' }}" data-locked-message="{{ $statusJendelaPresensi['pesan'] }}">
        ...
        <section class="room-strip" aria-label="Informasi ruang ujian">
            <div class="room-block"><span>Jadwal</span><strong>{{ $jadwal?->tanggal?->locale('id')->translatedFormat('l, d F Y') ?: '-' }}{{ $jadwal ? ' · '.$jadwal->labelWaktu() : '' }}</strong></div>
            <div class="room-block highlight"><span>Nomor ruang</span><strong>{{ $ruangUjianCbt->kode }}</strong></div>
        ...
        @unless($statusJendelaPresensi['dibuka'])
            <section class="schedule-guard" role="alert">
                <span class="schedule-guard-icon" aria-hidden="true">!</span>
                <div>
        ...
        <div class="scan-layout">
            <div>
                <section class="panel">
                    ...
                    <div id="camera-wrap" class="camera-wrap"><video id="camera-video" autoplay muted playsinline></video><canvas id="camera-canvas" hidden></canvas><div class="camera-placeholder"><strong>Kamera belum dinyalakan</strong><p>Tekan Mulai kamera dan izinkan penggunaan kamera belakang HP.</p></div><div class="scan-frame" aria-hidden="true"></div><div class="camera-message">Posisikan QR di dalam kotak</div></div>
                    <div class="camera-controls"><button id="start-camera" class="button button-primary button-grow" type="button" @disabled(! $statusJendelaPresensi['dibuka'])>Mulai kamera</button><button id="switch-camera" class="button button-secondary" type="button" disabled>Ganti kamera</button><button id="stop-camera" class="button button-danger" type="button" disabled>Hentikan</button></div>
                    <p id="camera-warning" class="camera-warning" hidden></p>
                </section>
