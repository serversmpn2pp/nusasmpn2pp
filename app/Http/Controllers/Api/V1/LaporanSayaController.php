...
class LaporanSayaController extends Controller
{
    public function index(Request $request, LaporanSiswaMobileService $service): JsonResponse
    {
        return $this->tanpaCache([
            'data' => $service->daftarSaya($request->user(), $this->validasiFilter($request)),
        ]);
    }
    ...
    public function show(
        ...
        return $this->tanpaCache([
            'data' => $service->rincianSaya($request->user(), $laporanPembinaanSiswa),
        ]);
    }
