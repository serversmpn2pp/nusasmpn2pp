...
class AkunOrangTuaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (! in_array($statusAkun, ['semua', 'aktif', 'nonaktif', 'belum', 'tanpa_nisn'], true)) {
            $statusAkun = 'semua';
        }
                    ...
                    ->when($kataKunci, function ($query) use ($kataKunci) {
                        $query->where(function ($query) use ($kataKunci) {
                            $query->whereLike('nama_lengkap', '%'.$kataKunci.'%')
                                ...
                                ->orWhereHas('orangTuaWali', fn ($query) => $query
                                    ->whereLike('nama_lengkap', '%'.$kataKunci.'%'));
                        });
                    })
                    ->when($statusAkun === 'aktif', fn ($query) => $query
                        ->whereHas('orangTuaWali.pengguna', fn ($query) => $query->where('aktif', true)))
                    ->when($statusAkun === 'nonaktif', fn ($query) => $query
                        ->whereHas('orangTuaWali.pengguna', fn ($query) => $query->where('aktif', false)))
        ...
        return view('akun-orang-tua.index', compact(
            'tahunPelajaranAktif',
            'daftarKelas',
            'kelasDipilih',
            'kelasId',
    ...
    {
        $pengguna = $this->akunOrangTuaService->buat($siswa);

        return back()->with(
            'berhasil',
            'Akun orang tua '.$siswa->nama_lengkap.' berhasil dibuat dengan username '.$pengguna->username.'.',
        );
    }
    ...
    public function storeMassal(Kelas $kelas)
    {
        ...
        return back()
            ->with('berhasil', 'Pembuatan akun orang tua kelas '.$kelas->nama.' selesai.')
            ->with('ringkasan_akun_orang_tua', $ringkasan);
    }

    public function resetPassword(Pengguna $pengguna)
    {
        $this->akunOrangTuaService->resetKataSandi($pengguna);
    ...
    {
        abort_unless($pengguna->akunOrangTua(), 404);
    ...
    public function cetak(Request $request, Kelas $kelas)
    {
        ...
        $anggotaKelas = AnggotaKelas::query()
            ->with('siswa.orangTuaWali.pengguna')
            ->where('kelas_id', $kelas->id)
            ->where('status_keanggotaan', 'aktif')
            ->whereHas('siswa', fn ($query) => $query->where('aktif', true))
            ->orderByRaw('nomor_absen IS NULL')
        ...
        return view('akun-orang-tua.cetak', compact('kelas', 'anggotaKelas'));
    }
    ...
    private function queryKelasYangDapatDiakses(?Pengguna $pengguna): Builder
    {
        ...
        if ($pengguna?->administrator() || $pengguna?->memilikiIzin('akun_orang_tua.kelola')) {
            return $query;
        }
