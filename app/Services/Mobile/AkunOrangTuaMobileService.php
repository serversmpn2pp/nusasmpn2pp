...
class AkunOrangTuaMobileService
{
    public function __construct(private readonly AkunOrangTuaService $akunOrangTuaService) {}
    ...
    public function daftar(Pengguna $pengguna, array $filter): array
    {
        ...
        $query = AnggotaKelas::query()
            ->with(['siswa.orangTuaWali.pengguna', 'kelas.tahunPelajaran'])
                    ...
                    ->when($kataKunci !== '', function (Builder $query) use ($kataKunci) {
                        ...
                        $query->where(function (Builder $query) use ($pola, $polaIdentitas) {
                            $query->whereRaw('LOWER(nama_lengkap) LIKE ?', [$pola])
                                ...
                                ->orWhereHas('orangTuaWali', fn (Builder $query) => $query
                                    ->whereRaw('LOWER(nama_lengkap) LIKE ?', [$pola])
                                    ->orWhereHas('pengguna', fn (Builder $query) => $query
                                        ->whereRaw('LOWER(username) LIKE ?', [$pola])));
                        });
                    })
                    ->when($statusAkun === 'aktif', fn (Builder $query) => $query
                        ->whereHas('orangTuaWali.pengguna', fn (Builder $query) => $query->where('aktif', true)))
                    ->when($statusAkun === 'nonaktif', fn (Builder $query) => $query
                        ->whereHas('orangTuaWali.pengguna', fn (Builder $query) => $query->where('aktif', false)))
                    ->when($statusAkun === 'belum', fn (Builder $query) => $query
                        ->whereDoesntHave('orangTuaWali')
                        ->whereNotNull('nisn')
                        ->where('nisn', '<>', ''))
    ...
    {
        return $this->akunOrangTuaService->buat($siswa);
    }
    ...
    public function buatMassal(Pengguna $pengguna, Kelas $kelas): array
    {
        ...
        foreach ($anggota as $item) {
            $siswa = $item->siswa;
            if (! $siswa || $siswa->orangTuaWali->isNotEmpty()) {
                $ringkasan['dilewati']++;
            ...
            try {
                $this->akunOrangTuaService->buat($siswa);
                $ringkasan['dibuat']++;
    ...
    public function resetKataSandi(Siswa $siswa): Pengguna
    {
        ...
        $this->akunOrangTuaService->resetKataSandi($akun);
    ...
    private function konteksKelas(Pengguna $pengguna): array
    {
        $dapatMengaksesSemua = $pengguna->administrator()
            || $pengguna->memilikiIzin('akun_orang_tua.kelola');
    ...
    {
        $siswa->loadMissing('orangTuaWali.pengguna');
        $orangTua = $this->orangTuaUtama($siswa);
        $akun = $orangTua?->pengguna;

        if (! $akun) {
            throw ValidationException::withMessages([
                'akun' => 'Siswa ini belum terhubung dengan akun orang tua.',
            ]);
        }

        abort_if($akun->akun_sistem || ! $akun->akunOrangTua(), 403);
    ...
    private function siapkanAnggota(
        ...
        return [
            ...
            'orang_tua' => $orangTua ? [
                ...
                'hubungan' => $orangTua->pivot?->hubungan,
                'utama' => (bool) ($orangTua->pivot?->utama ?? false),
            ] : [
                'tersedia' => false,
                'id' => null,
                'nama' => null,
            ...
            'kontak_keluarga' => $sertakanDetail
                ? $this->siapkanKontakKeluarga($siswa)
                : [],
            ...
            'akun' => $akun ? [
                ...
                'wajib_ganti_kata_sandi' => $sertakanDetail
    ...
    private function orangTuaUtama(Siswa $siswa): ?OrangTuaWali
    {
        return $siswa->orangTuaWali
            ->sortByDesc(fn (OrangTuaWali $orangTua) => (bool) ($orangTua->pivot?->utama ?? false))
            ->first();
    }

    private function siapkanKontakKeluarga(Siswa $siswa): array
    {
        return collect([
            ['kode' => 'ayah', 'label' => 'Ayah', 'nama' => $siswa->nama_ayah, 'nomor_wa' => $siswa->nomor_wa_ayah],
            ['kode' => 'ibu', 'label' => 'Ibu', 'nama' => $siswa->nama_ibu, 'nomor_wa' => $siswa->nomor_wa_ibu],
            ['kode' => 'wali', 'label' => $siswa->hubungan_wali ?: 'Wali', 'nama' => $siswa->nama_wali, 'nomor_wa' => $siswa->nomor_wa_wali],
        ])->map(fn (array $kontak) => [
            ...$kontak,
            'utama' => $siswa->kontak_absensi_utama === $kontak['kode'],
        ])->values()->all();
    }
    ...
    private function ringkasan(Collection $kelasIds, int $kelasId): array
    {
        ...
        return [
            ...
            'akun_aktif' => (clone $query)
                ->whereHas('orangTuaWali.pengguna', fn (Builder $query) => $query->where('aktif', true))
                ->count(),
            'akun_nonaktif' => (clone $query)
                ->whereHas('orangTuaWali.pengguna', fn (Builder $query) => $query->where('aktif', false))
                ->count(),
    ...
    {
        return [
            'dapat_kelola' => $pengguna->memilikiIzin('akun_orang_tua.kelola'),
