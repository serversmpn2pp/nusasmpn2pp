...
class TerapkanNilaiCbtService
{
    public function __construct(
        private PublikasiNilaiService $publikasiNilai,
        private KoreksiOtomatisCbtService $koreksiOtomatis,
    ) {}
    ...
    public function terapkan(UjianCbt $ujianCbt, ?int $penggunaId): array
    {
        ...
        $this->koreksiOtomatis->koreksiUjian($ujianCbt);
        ...
        DB::transaction(function () use (
            ...
            foreach ($pesertaUjian as $peserta) {
                ...
                $perluKoreksiManual = collect($soalManualIds)
                    ->filter(fn ($id) => $jawaban->has($id)
                        && ! is_null($jawaban[$id]->jawaban)
                        && is_null($jawaban[$id]->skor))
                    ->count();
                ...
                $skorTotal = $jawaban
                    ->filter(fn (JawabanPesertaUjianCbt $item) => in_array(
                        (int) $item->soal_ujian_cbt_id,
                        $soalUjianIds,
                        true,
                    ))
                ...
                $peserta->update([
                    ...
                    'nilai_diterapkan_oleh_pengguna_id' => $penggunaId,
                ]);
        ...
        return [
            'ringkasan' => $ringkasan,
            'pesan' => $this->pesan($ujianCbt, $ringkasan),
        ];
    }

    private function pesan(UjianCbt $ujianCbt, array $ringkasan): string
    {
