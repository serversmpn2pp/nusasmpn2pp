...
class EmployeeScanServerCard extends StatelessWidget {
  const EmployeeScanServerCard({required this.dashboard, super.key});

  final EmployeeScanStatusDashboard dashboard;
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            runSpacing: 7,
            children: [
              _Pill(label: schedule.phaseLabel, color: phaseColor),
              _Pill(
                label: '${schedule.count} jadwal aktif',
                color: NusaColors.accent,
              ),
            ],
          ),
          ...
          if (schedule.available) ...[
            ...
            Container(
              ...
              child: Text(
                'Rentang masuk ${schedule.checkInWindow}  |  '
                'rentang pulang ${schedule.checkOutWindow}',
...
class EmployeeScanSummaryGrid extends StatelessWidget {
  const EmployeeScanSummaryGrid({required this.summary, super.key});
...
class EmployeeScanHealthCard extends StatelessWidget {
  const EmployeeScanHealthCard({required this.summary, super.key});
...
class EmployeeScanFilters extends StatelessWidget {
  const EmployeeScanFilters({
  ...
  Widget build(BuildContext context) => Column(
    children: [
      NusaTextField(
        fieldKey: const Key('employee-scan-status-search'),
        controller: searchController,
        hintText: 'Cari pegawai, NIP, jenis, atau status',
        ...
        builder: (context, constraints) {
          final fields = [
            NusaDropdownField<String>(
              fieldKey: const Key('employee-scan-status-type-filter'),
              ...
              options: [
                ...
                for (final type in employeeTypes)
                  NusaDropdownOption(value: type, label: type),
              ],
              onChanged: (value) => onEmployeeTypeChanged(
                value?.trim().isEmpty == true ? null : value,
              ),
            ),
            NusaDropdownField<String>(
              fieldKey: const Key('employee-scan-status-result-filter'),
              value: status,
              decoration: const InputDecoration(
...
class EmployeeScanActivityHeader extends StatelessWidget {
  const EmployeeScanActivityHeader({required this.count, super.key});
...
class EmployeeScanActivityCard extends StatelessWidget {
  const EmployeeScanActivityCard({required this.activity, super.key});

  final EmployeeScanActivity activity;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      key: Key('employee-scan-activity-${activity.id}'),
      ...
      child: Column(
        ...
        children: [
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _EmployeeAvatar(employee: employee, color: color),
              const SizedBox(width: 10),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(
                      employee?.name ?? 'Kartu tidak dikenali',
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: const TextStyle(
                        fontSize: 14,
          ...
          if ((activity.attendance?.lateMinutes ?? 0) > 0 ||
              ...
              activity.attendance?.scheduleName?.trim().isNotEmpty == true) ...[
            ...
            Wrap(
              ...
              children: [
                ...
                if (activity.attendance?.scheduleName?.trim().isNotEmpty ==
                    true)
                  _AttendanceTag(
                    icon: Icons.event_available_outlined,
                    label: activity.attendance!.scheduleName!,
                  ),
              ],
            ),
          ],
...
class _EmployeeAvatar extends StatelessWidget {
  const _EmployeeAvatar({required this.employee, required this.color});
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: employee?.photoUrl?.trim().isNotEmpty == true
        ? Image.network(
            employee!.photoUrl!,
            fit: BoxFit.cover,
            errorBuilder: (context, error, stackTrace) =>
                _AvatarFallback(employee: employee, color: color),
          )
        : _AvatarFallback(employee: employee, color: color),
  );
}
...
class _AvatarFallback extends StatelessWidget {
  const _AvatarFallback({required this.employee, required this.color});
...
class EmployeeScanStatusEmpty extends StatelessWidget {
  const EmployeeScanStatusEmpty({super.key});
  ...
  Widget build(BuildContext context) => const Padding(
    ...
    child: Column(
      ...
      children: [
        ...
        SizedBox(height: 11),
        Text(
          'Belum ada aktivitas scan pegawai yang sesuai dengan filter.',
...
class EmployeeScanStatusError extends StatelessWidget {
  const EmployeeScanStatusError({
...
String _employeeMeta(ScannedEmployee? employee) {
  if (employee == null) return 'Periksa kartu dan mesin scanner';
  return [
    if (employee.type?.trim().isNotEmpty == true) employee.type!,
    if (employee.nip?.trim().isNotEmpty == true) employee.nip!,
  ].join(' · ');
}
