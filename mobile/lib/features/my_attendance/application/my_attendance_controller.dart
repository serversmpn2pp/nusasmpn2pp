...
import 'package:nusa/features/my_attendance/data/my_attendance_repository.dart';
...
class MyAttendanceController extends AsyncNotifier<MyAttendancePage> {
  ...
  @override
  Future<MyAttendancePage> build() => _fetch();
  ...
  Future<MyAttendancePage> _fetch() => _guard(() async {
    final result = await ref
        .read(myAttendanceRepositoryProvider)
        .fetch(
...
final myAttendanceControllerProvider =
    AsyncNotifierProvider.autoDispose<MyAttendanceController, MyAttendancePage>(
      MyAttendanceController.new,
    );
