...
import 'package:nusa/features/my_survey_results/data/my_survey_results_repository.dart';
import 'package:nusa/features/survey_monitoring/domain/survey_monitoring.dart';

class MySurveyResultsController extends AsyncNotifier<SurveyMonitoringPage> {
  int? _academicYearId;
  ...
  Future<SurveyMonitoringPage> _fetch({required int page}) async {
    try {
      final result = await ref
          .read(mySurveyResultsRepositoryProvider)
          .fetch(
...
final mySurveyResultsControllerProvider =
    AsyncNotifierProvider.autoDispose<
      MySurveyResultsController,
      SurveyMonitoringPage
    >(MySurveyResultsController.new);

typedef MySurveyResultDetailQuery = ({int assignmentId, String semester});

final mySurveyResultDetailProvider = FutureProvider.autoDispose
    .family<SurveyMonitoringDetail, MySurveyResultDetailQuery>((
      ref,
      query,
    ) async {
      try {
